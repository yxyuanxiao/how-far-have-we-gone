...
def parse_args():
    parser = argparse.ArgumentParser(description="Compute SRCC / PLCC between predicted and ground-truth scores")
    ...
    parser.add_argument("--output_csv", type=str, default=None,
                        help="If set, save per-image (id, pred_score, gt_score) to this CSV file.")
...
def main():
    ...
    for pred_path, gt_path in zip(pred_paths, gt_paths):
        ...
        for pred_meta, gt_meta in zip(pred_metas, gt_metas):
            ...
            all_results.append({"id": pred_meta["id"], "pred_score": pred_score, "gt_score": gt_meta["gt_score"]})
    ...
    if args.output_csv is not None:
        with open(args.output_csv, mode="w", newline="", encoding="utf-8") as f:
            writer = csv.DictWriter(f, fieldnames=["id", "pred_score", "gt_score"])
            writer.writeheader()
            writer.writerows(all_results)
        print(f"Predicted scores saved to {args.output_csv}")
