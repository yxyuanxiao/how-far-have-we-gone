...
def main(cfg):
    ...
    for img, mos_str, std_str in zip(imgs, moses, stds):
        ...
        if img in split["train"]:
            training = True
        elif img in split["test"]:
            training = False
        else:
            idx += 1
            continue
    ...
    print("=" * 100)
    os.makedirs(os.path.dirname(save_train) or ".", exist_ok=True)
    os.makedirs(os.path.dirname(save_test) or ".", exist_ok=True)
